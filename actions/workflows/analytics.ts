...
import { periodToDateRange } from "@/lib/helper/helper";
import prisma from "@/lib/prisma";
import { ExecutionPhaseStatus, Period, WorkflowExecutionStatus, WorkflowExecutionType } from "@/types/workflow";
...
export async function getPeriods() {
  ...
  const years = await prisma.workflowExectution.aggregate({
    where: {
      userId,
    },
    _min: {
...
export async function getStatsCardsValue(period: Period) {
  ...
  const executions = await prisma.workflowExectution.findMany({
    where: {
      userId,
      startedAt: {
...
export async function getCreditsUsageInPeriod(period: Period) {
  ...
  const executionsPhases = await prisma.workflowExectution.findMany({
    where: {
      userId,
      startedAt: {
        gte: dateRange.startDate,
